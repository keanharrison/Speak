...
/** Apple-style continuous-corner app tile */
const appTile =
  "inline-flex h-14 w-14 shrink-0 items-center justify-center rounded-[13px] shadow-[0_1px_2px_rgba(0,0,0,0.12)]";
...
function CopyableEmail({ email }: { email: string }) {
  ...
  return (
    <div ref={rootRef} className="relative inline-block max-w-full">
      <a
        href={`mailto:${email}`}
        className="inline-flex min-h-[44px] max-w-full items-center break-all text-[14px] font-medium text-[#0A0A0A] underline-offset-2 hover:underline"
        onContextMenu={(e) => {
          e.preventDefault();
          ...
          {data.people.map((person) => (
            <li key={person.email} className="glass-panel px-4 py-4">
              <div className="flex items-start justify-between gap-3">
                ...
                >
                  <LinkedInGlyph className="h-7 w-7 text-white" />
                </AppIconLink>
              </div>
            </li>
          ))}
        ...
        <div className="mt-4 flex items-center gap-4">
          {data.socials.map((social) => {
            ...
            return (
              ...
              >
                <XGlyph className="h-6 w-6 text-white" />
              </AppIconLink>
            );
          })}
        </div>
