...
/** Second thing on Home — resume recent Speak threads. */
      ...
      <ul className="mt-3 flex flex-col gap-2.5">
        {chats.map((chat) => (
          <li key={chat.id}>
            <Link
              href={chat.href}
              className="glass-panel flex w-full min-h-[44px] min-w-0 items-center gap-3 overflow-hidden px-4 py-3.5 text-left"
            >
              <div className="min-w-0 flex-1">
                <p className="text-[15px] font-semibold text-[#0A0A0A]">
                  <span className="text-[#6b6b6b]">{chat.preview}</span>
